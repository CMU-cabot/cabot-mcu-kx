...
void task_sequence(void *pvParameters)
{
  while(1)
  {
    ...
    else if(flag_power_on == true && flag_shutdown == true)
    {
      ...
      {
        //pc shutdown detect
        if(sequence_cnt == SHUTDOWN_PC && digitalRead(PC_ENABLE) == LOW)
        {
          vTaskDelay(5000);
          break;
        }
...
void setup()
{
  ...
  CAN0.init_Filt(1,0,CAN_FILTER1);
  CAN0.init_Mask(1,0,CAN_MASK1);
  CAN0.init_Filt(2,0,CAN_FILTER2);
...
void loop()
{
  ...
  //for prototype model
  #ifdef PROTO
  if(digitalRead(PC_ENABLE) == LOW && flag_sequencing == false && flag_power_on == true)
  {
    if(reboot_cnt >= 2000)
    {
      reboot_cnt = 0;
  ...
  else if(digitalRead(PC_ENABLE) != LOW)
  {
    reboot_cnt = 0;
  }
  #endif
